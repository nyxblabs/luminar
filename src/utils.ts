 ...
 */
function isReadonlyArray(array: readonly any[] | any): array is readonly unknown[] {
   return Array.isArray(array)
}

export function parseLuminarType(luminarSchema: LuminarTypeOrSchema): [parser: TypeFunction, isArray: boolean] {
...
export function normalizeBoolean<T>(parser: TypeFunction,
   value: T) {
   if (parser === Boolean)
      return value !== 'false'
...
export function applyParser(typeFunction: TypeFunction,
   value: any) {
...
function validateLuminarName(luminarName: string) {
...
interface LuminarRegistry {
   [luminarName: string]: LuminarParsingData
}

export function createRegistry(schemas: Luminars) {
...
export function finalizeLuminars(schemas: Luminars,
