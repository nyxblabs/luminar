...
export interface Luminars<ExtraOptions = Record<string, unknown>> {
...
> = (
   Luminar extends (TypeFunctionArray<infer T> | LuminarSchema<TypeFunctionArray<infer T>>)
      ? (Luminar extends LuminarSchemaDefault<TypeFunctionArray<T>, infer D> ? T[] | D : T[])
      : (Luminar extends TypeFunction<infer T> | LuminarSchema<TypeFunction<infer T>>
            ? (Luminar extends LuminarSchemaDefault<TypeFunction<T>, infer D> ? T | D : T | undefined) : never)
)

export interface ParsedLuminars<Schemas = Record<string, unknown>> {
   luminars: Schemas
   unknownLuminars: {
...
export interface TypeLuminarOptions {
   /**
